<x-app-layout>
                ...
                <div class="md:w-96 flex flex-col border-l border-purple-500/20">
                    ...
                    <div class="border-t border-purple-500/20 p-4">
                        <div class="flex items-center gap-4 mb-3">
                            ...
                            <!-- Share Button -->
                            <button onclick="sharePost({{ $post->id }}, '{{ $post->user->name }}')" class="text-gray-300 hover:text-purple-400 transition hover:scale-110 transform">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-7 h-7">
                            ...
                            <form action="{{ route('posts.save', $post) }}" method="POST" class="save-form ml-auto">
                                @csrf
                                <button type="submit" class="save-btn transition hover:scale-110 transform {{ $post->isSavedBy(auth()->user()) ? 'text-purple-500' : 'text-gray-300 hover:text-purple-500' }}">
    ...
    <script>
        let currentShareUrl = '{{ route('posts.show', $post) }}';
        let currentShareTitle = 'Check out this post by {{ $post->user->name }}';
